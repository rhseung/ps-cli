...
### `solving` - 풀이 중인 문제 목록 및 열기

solving 디렉터리에 있는 풀이 중인 문제 목록을 표시하고, 선택한 문제를 에디터로 엽니다.
...
- solving 디렉터리의 각 문제 디렉터리를 meta.json에서 제목, 티어 등 정보를 읽어 표시합니다
- `ls solving`으로는 문제 번호만 보이던 것을, 제목과 티어를 포함한 직관적인 목록으로 확인할 수 있습니다
- 목록에서 문제를 선택하면 해당 문제 디렉터리를 에디터로 엽니다
